...
export default function Week06TopicDetail() {
  ...
  useEffect(() => {
    if (topic) {
      document.title = `Week 06 · ${topic.title} — Ryman Curriculum`;
    } else {
  ...
  if (!topic) {
    return (
      <div style={containerStyle}>
        <Link to="/week/06" style={backButtonStyle}>
          ← Back to Week 06
        </Link>
    ...
    <div style={containerStyle}>
      ...
      <div style={sourceTagStyle}>
        {topic.pccSources.length > 0
          ? `Adapted from PCC DMA 10: ${topic.pccSources.join(' · ')}`
      ...
      <TopicNav topicList={topicList} topicKey={topicKey} weekNum="06" />
    </div>
  );
}
